...
public class ArbreRNController {
    ...
    private void supprimerElem(ActionEvent event) {
               ...
               if(arn.racine.recherche(numbre)){
                    ...
                    tracer_arn(arn.racine,null);
                    label.setText("Le nombre Ã  été supprimé");
               }
    ...
    private void ajouterElem(ActionEvent event) {
                int number=Integer.parseInt(champ_text1.getText());
                arn.ajout(number);
                label.setText("le nombre "+number+" a été ajouté");
                tracer_arn(arn.racine,null);
    }
	...
	private void tracer_arn(Noeud rac,Comparable o){
        if(!rac.isSentinelle()){
            pan.getChildren().clear();
            if(o==null) 
        ...
        private float tracer_arn(float x1,float x2,Noeud rac,float y){
        float xd=0,xg=0;
        ...
        private float tracer_after_search(float x1,float x2,Noeud rac,float y,@SuppressWarnings("rawtypes") Comparable o){
         float xd=0,xg=0;
        int nbFeuille = GetnbFeuille(rac);
        int nbFeuilleG = GetnbFeuille(rac.gauche);
        float x = (nbFeuilleG*100/nbFeuille)*(x2-x1)/100 + x1;
    ...
    private int GetnbFeuille(Noeud r){
